...
function MedicalHistory({
  ...
  const [medicineListAPiData, setmedicineListAPiData] = React.useState<
    string[]
  >([]);
  ...
  const [anethesiaList, setAnesthesiaList] = React.useState<string[]>([]);
  const getAllList = async () => {
    ...
    const anesthesiaResponse = await authFetch("data/anesthesia", user.token);
    if (anesthesiaResponse.message == "success") {
      setAnesthesiaList(anesthesiaResponse.anesthesia);
    }
  };
  ...
  const [anethesiaAllergy, setAnaethesiaAllergy] =
    React.useState<searchListType>({
      searchedList: [],
      selectedList: medicalHistoryData.anaesthesia
        ? medicalHistoryData.anaesthesia.split(",")
        : [],
      search: "",
      istrue: medicalHistoryData.anaesthesia ? true : false,
    });
  ...
  React.useEffect(() => {
    if (!anethesiaAllergy.search) {
      setAnaethesiaAllergy((prevvalue) => {
        ...
        return {
          ...
          searchedList: [
            ...medicineList.filter((el) =>
              el.toLowerCase().includes(anethesiaAllergy.search.toLowerCase())
            ),
          ],
        };
      });
    }
  }, [anethesiaAllergy.search, anethesiaList]);
  ...
  React.useEffect(() => {
    setMedicalHistoryData({
      ...
      anaesthesia: anethesiaAllergy.selectedList.join(","),
  ...
  React.useEffect(() => {
    if (!medicineAllergy.search) {
      setMedicineAllergy((prevvalue) => {
        return { ...prevvalue, searchedList: [...medicineListAPiData] };
      });
    } else {
      setMedicineAllergy((prevValue) => {
        return {
  ...
  const fetchMedicineList = async () => {
    if (medicineAllergy?.search?.length == 3) {
      const text = medicineAllergy?.search;
      const response = await authPost("data/medicines", { text }, user.token);

      if (response.message === "success") {
        const allData = response.medicines?.map(
          (medicine: { Medicine_Name: string }) => medicine.Medicine_Name
        );

        setmedicineListAPiData(allData);
      }
    }
  };
            ...
            control={
              <Checkbox
                onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
                  if (event.target.checked) {
                    setDisease((el) => {
                      return [...el, "Hyperlipidemia / Dyslipidemia"];
                    });
                  }
                  if (!event.target.checked) {
                    setDisease((el) => {
                      return el.filter(
                        (dis) => dis != "Hyperlipidemia / Dyslipidemia"
                      );
                    });
                  }
                }}
            ...
            <Grid container xs={12} alignItems={"center"} spacing={2}>
              <Grid item xs={10}>
                <Autocomplete
                  ...
                  options={medicineListAPiData}
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      placeholder="Enter 3 letters in medicine"
                      label="Medicine Name"
                      required
                    />
                  )}
      ...
      <Grid xs={12} item>
        <Stack spacing={2} direction="column" rowGap={2}>
          ...
          >
            <Chip
              label={"Yes"}
              onClick={() =>
                setAnaethesiaAllergy((pre) => {
                  return { ...pre, istrue: true };
                })
              }
              color={anethesiaAllergy.istrue ? "primary" : "default"}
              sx={{ boxSizing: "border-box", padding: "10px 20px" }}
              disabled={formDisabled}
            />
            <Chip
          ...
          {anethesiaAllergy.istrue ? (
            <Grid container xs={12} alignItems={"center"} spacing={2}>
              <Grid item xs={10}>
                <Autocomplete
                  freeSolo // Allow the user to input a value that's not in the options list
                  value={anethesiaAllergy.search}
                  onChange={(_, newValue: string | null) => {
                    setAnaethesiaAllergy((prev) => ({
                      ...prev,
                      search: newValue || "",
                    }));
                  }}
                  inputValue={anethesiaAllergy.search || undefined}
                  onInputChange={(_, newInputValue) => {
                    setAnaethesiaAllergy((prev) => ({
                      ...prev,
                      search: newInputValue || "",
                  ...
                  options={anethesiaList}
                  renderInput={(params) => (
                    <TextField {...params} label="Anaesthesia Name" required />
                  )}
                />
              </Grid>
              <Grid xs={2} item>
                <Button
                  ...
                  onClick={() => {
                    if (
                      anethesiaAllergy.search &&
                      !anethesiaAllergy.selectedList.includes(
                        anethesiaAllergy.search
                      )
                    ) {
                      setAnaethesiaAllergy((prev) => ({
                    ...
                    setAnaethesiaAllergy((prev) => ({ ...prev, search: "" }));
                  }}
                  endIcon={<AddIcon />}
                >
                  Add
        ...
        <Grid item xs={12}>
          <Stack
            ...
            flexWrap={"wrap"}
          >
            {anethesiaAllergy.selectedList.map((el) => {
              return (
                <Chip
                  label={el}
                  onDelete={() => {
                    setAnaethesiaAllergy((curr) => {
                      return {
                        ...curr,
                        selectedList: curr.selectedList.filter(
                          (val) => val != el
