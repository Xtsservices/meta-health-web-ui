...
function AddReport({ files, setFiles, consentText }: reportType) {
  ...
  const handleSubmit = async () => {
    if (files?.length) {
      ...
      const reportResponse = await authPost(
        `attachment/${user.hospitalID}/${timeline.id}/${timeline.patientID}/${user.id}/consentform`,
        form,
        user.token
      );
      if (reportResponse.message == "success") {
        dispatch(setSuccess("Consentform successfully uploaded"));
      } else {
