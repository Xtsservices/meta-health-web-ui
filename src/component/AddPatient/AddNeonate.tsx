...
function AddNeonateEmergency() {
  ...
  const getAlreadyExistedData = async () => {
    ...
    if (response.message == "success") {
      if (response.status == 0) {
        dispatch(setError("Record already exists"));
  ...
  useEffect(() => {
    const createPostData = async () => {
      ...
      setBasicObj((state) => {
        ...
        state.category = patientCategory.neonate;
    ...
    createPostData();
  }, [basicFormData, ptype, user.hospitalID]);
  ...
  const handleBasicFormSubmit = async () => {
    ...
    console.log("checkkk", patientBasicDetailform);
...
export default AddNeonateEmergency;
