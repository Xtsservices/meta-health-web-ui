...
import { PatientType, medicalHistoryFormType } from "../../../../types";
...
const Summary = ({ medicalHistory, currentPatient }: propType) => {
  // const dietList = [
  //   "Protein-rich foods",
  //   "Leafy vegetables",
  //   "Fruits",
  ...
  // console.log("from summary", currentPatient.diet);
  return (
    <div>
      {medicalHistory ? (
        <>
      ...
      <View style={{ flex: 1 }}>
        ...
        >
          {currentPatient.advice || "No Advice"}
        </Text>
      </View>
      <View
        style={{ flexDirection: "row", alignItems: "center", marginLeft: 60 }}
      >
        <View style={{ marginTop: 30 }}>
          ...
          {currentPatient?.diet
            ? currentPatient.diet?.split(",").map((item, index) => (
                <View
        ...
        <View>
          ...
          <Text style={{ fontSize: 10, marginLeft: 200, marginTop: 5 }}>
            {currentPatient.followUpDate
              ? new Date(currentPatient.followUpDate).toLocaleDateString(
                  "en-GB"
                )
