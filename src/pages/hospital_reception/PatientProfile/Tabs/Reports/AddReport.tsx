...
import file_icon from "../../../../../assets/reception/file_icon.png";
          ...
          if (files) {
            Array.from(files).forEach((file) => {
              if (file.type === "video/mp4" && file.size > 25 * 1024 * 1024) {
                //  dispatch(setError(`The file ${file.name} exceeds the 25MB limit for video files.`));
              } else if (
                file.type === "audio/mpeg" &&
                file.size > 1 * 1024 * 1024
              ) {
                //   dispatch(setError(`The file ${file.name} exceeds the 1MB limit for audio files.`));
              } else {
                handleFile(file);
              }
            });
