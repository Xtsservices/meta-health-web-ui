...
import NoLatestPatientDashboardData from "../dashboard_opd/NoLatestPatientDashboardData";
...
export default function BasicTable() {
  ...
  async function handlePatientView(id: number | null) {
    ...
    if (response.message == "success") {
      return navigate(`/hospital-dashboard/inpatient/list/${id}`);
    }
  }
  const getNurseAddedData = async () => {
    const response = await authFetch(
      `patient/${user.hospitalID}/patients/nurseRecent/${patientStatus.inpatient}?userID=${user.id}&role=${user.role}`,
      user.token
    );
    // dispatch(setLoading(false));
    if (response.message == "success") {
  ...
  return (
    <div className={styles.table}>
      <table >
        ...
        <tbody style = {{maxHeight:"450px", overflow:"scroll",scrollbarWidth:"thin"}}>
          {rows.length < 1 ? (
            <tr>
              <td colSpan={7}>
